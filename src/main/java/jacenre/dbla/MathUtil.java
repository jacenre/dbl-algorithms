...
public class MathUtil {
	...
	public static double LambertMinusOne(double input) throws IllegalArgumentException {
		if (input < -1 / FastMath.exp(1) || input >= 0) { // input not in [-1/e, 0)
			throw new IllegalArgumentException("Only defined on [-1/e, 0)");
		}

		final double sigma = -1 - FastMath.log(-input); // for simplification of final expression
		...
		return -1 - sigma
				- 2 / M1 * (1 - 1 / (1 + M1 * Math.sqrt(sigma / 2) / (1 + M2 * sigma * FastMath.exp(M3 * Math.sqrt(sigma)))));
	}

	/**
	 * Computes the Lambert value on the 0 branch with an error of at most 0.01%.
	 * Cfr.
	 * https://citeseerx.ist.psu.edu/viewdoc/download?doi=10.1.1.90.6481&rep=rep1&type=pdf
	 *
	 * @param input value to compute Lambert value for.
	 * @throws IllegalArgumentException if {@code input} not in domain [-1/e,
	 *                                  infinity).
	 */
	public static double LambertZero(double input) throws IllegalArgumentException {
		if (input < -1 / FastMath.exp(1)) { // input not in [-1/e, infinity)
			throw new IllegalArgumentException("Only defined on [-1/e, infinity)");
		}

		final double y = Math.sqrt(2 * FastMath.exp(1) * input + 2); // to simplify expression
		...
		return (2 * FastMath.log(1 + B * y) - FastMath.log(1 + C * FastMath.log(1 + D * y)) + E)
				/ (1 + 1 / (2 * FastMath.log(1 + B * y) + 2 * A));
	}
}
